...
class InvoiceApp:
    def __init__(self, master):
        ...
        self.btn_del = tk.Button(master, text="Delete Existing files",font=labelfont, width=15, bg='Purple',fg='White', command=threading.Thread(target=self.delete_files("invoice/")).start)
        self.btn_del.place(relx=0.550, rely=0.28)
        self.message_label = tk.Label(master, text="",bg="Sky blue",font=labelfont)
        self.message_label.place(relx=0.50, rely=0.19)
    #delete files from folder
    def delete_files(self,dir):
        
        for file in os.listdir(dir):
            os.remove(os.path.join(dir,file))
        self.message_label.configure(text="Existing files deleted") 
